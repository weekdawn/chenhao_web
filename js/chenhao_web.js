$(document).ready(function () {
    ...
    function show_table(cu_price_rate, plastic_price_rate) {
        ...
        if(type != "选择型号"){
            let select_type = costdata[type];
            $.each(select_type, function (j, i_item) {
                let specs = i_item["specs"];
                let cu_weight = parseFloat(i_item["cu_weight"]);    //铜重
                let plastic_weight = parseFloat(i_item["plastic_weight"]);  //塑料重
